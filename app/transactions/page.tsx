  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      if (cats.length === 0 && !cachedCats.length) {
        // Verificar se o banco realmente não possui nenhuma categoria antes de semear
        const allCatsInDB = await localDB.get('categories', user.uid);
        if (allCatsInDB.length === 0) {
